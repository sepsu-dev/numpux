    ...
    return (
        <>
            <div className="flex items-center justify-between gap-4 pb-6">
                <div>
                    <h2 className="text-xl font-bold text-foreground tracking-tight">Proyek</h2>
                    <p className="text-muted-foreground text-xs mt-0.5">Kelola inisiatif dan progress proyek tim Anda.</p>
                </div>

                <Link href="/projects/new">
                    <button className="flex items-center gap-1.5 px-3.5 py-2 bg-primary text-primary-foreground rounded-lg text-xs font-medium hover:bg-primary/90 transition-all shadow-sm">
                        <Plus size={14} />
                        Proyek Baru
                    </button>
                </Link>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    >
                                    ...
                                    <DropdownMenuContent align="end" className="rounded-lg border border-border bg-card font-sans text-xs shadow-sm">
                                        <DropdownMenuItem asChild>
                                            <Link href={`/projects/edit/${project.id}`} className="cursor-pointer py-1.5 px-2.5">
                                                <Edit size={13} className="mr-2" /> Ubah
                                            </Link>
                                        </DropdownMenuItem>
                                        <DropdownMenuItem
                                        ...
                                        >
                                            <Trash2 size={13} className="mr-2" /> Hapus
                                        </DropdownMenuItem>
                                    </DropdownMenuContent>
                                </DropdownMenu>
                            </div>
                        ...
                        <div className="mt-5 pt-4 border-t border-border/50 space-y-2">
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-muted-foreground font-medium">{project.tasks} Tugas</span>
                ...
                <Link href="/projects/new" className="group">
                    <div className="h-full min-h-[160px] border border-dashed border-border rounded-xl flex flex-col items-center justify-center p-6 text-center hover:border-primary/50 hover:bg-muted/30 transition-all">
                        ...
                        <h4 className="text-sm font-semibold text-foreground">Proyek Baru</h4>
                        <p className="text-xs text-muted-foreground mt-0.5">Tambah proyek baru ke tim.</p>
                    </div>
                </Link>
            </div>
            ...
            <Dialog open={deleteId !== null} onOpenChange={(open) => !open && setDeleteId(null)}>
                <DialogContent className="rounded-xl border border-border p-6 font-sans shadow-md bg-card max-w-sm">
                    <DialogHeader className="space-y-2">
                        ...
                        <DialogTitle className="text-lg font-semibold text-foreground tracking-tight">Hapus Proyek?</DialogTitle>
                        <DialogDescription className="text-xs text-muted-foreground">
                            Semua data di dalam proyek ini akan dihapus permanen. Lanjutkan?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="mt-6 flex gap-2">
                        ...
                        <form action={async () => { if (deleteId) await deleteProjectAction(deleteId); }} className="flex-1">
                            <Button type="submit" size="sm" className="w-full bg-red-500 hover:bg-red-600 text-white font-medium rounded-lg">Ya, Hapus</Button>
                        </form>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
