...
import type { Task, TaskStatus, Priority, TaskActivity } from "@/lib/types";
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-card/40 p-2.5 rounded-2xl border border-border/60">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
                    ...
                    {masterPriorities.map((p) => {
                        const active = selectedPriority === p.name;
                        return (
                            <button
                                key={p.id}
                                onClick={() => setSelectedPriority(p.name)}
                                className={cn(
                                    "px-2.5 py-1 text-[11px] font-medium rounded-lg border transition-all cursor-pointer whitespace-nowrap flex items-center gap-1.5",
                                    active
                                        ? "bg-foreground text-background border-foreground font-semibold shadow-xs"
                                        : "bg-card text-muted-foreground border-border hover:text-foreground hover:bg-muted/50"
                                )}
